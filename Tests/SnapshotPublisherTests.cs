...
public class SnapshotPublisherTests
{
    ...
    {
        var pub = new SnapshotPublisher();
        var snap = new SimSnapshot(5, 5.0 / 60.0, 0, Array.Empty<ColonistView>(), Array.Empty<SpotView>());
        pub.Publish(snap);
    ...
    public async Task Latest_publish_wins_under_concurrent_writes()
    {
        ...
        var writer = Task.Run(() =>
        {
            ...
            {
                pub.Publish(new SimSnapshot(i, i / 60.0, 0, Array.Empty<ColonistView>(), Array.Empty<SpotView>()));
            }
            done.Set();
        });
